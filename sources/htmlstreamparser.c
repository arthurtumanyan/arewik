...
HTMLSTREAMPARSER *html_parser_init() {
    return html_parser_reset((HTMLSTREAMPARSER *) malloc(sizeof (HTMLSTREAMPARSER)));
}
...
char *html_parser_rtrim(char *src, size_t *len) {
    while (ishtmlspace(src[(*len) - 1]) && *len > 0) {
        (*len)--;
    }
    return src;
}
...
char *html_parser_trim(char *src, size_t *len) {
    return html_parser_rtrim(html_parser_ltrim(src, len), len);
}
...
int html_parser_is_in(HTMLSTREAMPARSER *hsp, int html_part) {
    if (html_part >= 0 && html_part < HTML_PART_SIZE) return hsp->html_part[html_part];
    else return 0;
}
...
void html_parser_char_parse(HTMLSTREAMPARSER *hsp, const char c) {
    ...
    switch (*s) {
        case 0: // inside the inner text
            if (c == '<') {
                *s = 1;
                memset(h, 0, HTML_PART_SIZE);
                h[HTML_TAG] = 1;
                h[HTML_TAG_BEGINNING] = 1;
            } else {
                if (!h[HTML_INNER_TEXT]) {
                    memset(h, 0, HTML_PART_SIZE);
                    h[HTML_INNER_TEXT] = 1;
                    h[HTML_INNER_TEXT_BEGINNING] = 1;
        ...
        case 1: // tag beginning
            ...
            *l = 0;
            if (c == '/') {
                *s = 2;
                h[HTML_CLOSING_TAG] = 1;
                h[HTML_SLASH] = 1;
                h[HTML_NAME] = 1;
                h[HTML_NAME_BEGINNING] = 1;
            } else if (c == '!') {
                *s = 9;
                h[HTML_COMMENT] = 1;
            } else if (c == '>') {
        ...
        case 5: // before an attribute value
            ...
            if (!ishtmlspace(c)) {
                ...
                if (c == '>') {
                    ...
                    *s = 6;
                    h[HTML_VALUE_QUOTED] = 1;
                    h[HTML_VALUE_DOUBLE_QUOTED] = 1;
                } else if (c == '\'') {
                    *s = 7;
                    h[HTML_VALUE_QUOTED] = 1;
                    h[HTML_VALUE_SINGLE_QUOTED] = 1;
                } else {
                    *s = 8;
                    h[HTML_VALUE] = 1;
                    h[HTML_VALUE_BEGINNING] = 1;
                }
            }
            break;
    ...
    if (h[HTML_INNER_TEXT]) {
        if (h[HTML_INNER_TEXT_BEGINNING]) {
            hsp->inner_text_len = 0;
            hsp->inner_text_real_len = 0;
        }
...
size_t html_parser_tag_length(HTMLSTREAMPARSER *hsp) {
    if (!hsp->html_part[HTML_NAME] && hsp->parser_state != 12 && hsp->parser_state != 13) return hsp->tag_name_len;
    else return 0;
}

size_t html_parser_tag_real_length(HTMLSTREAMPARSER *hsp) {
    return hsp->tag_name_real_len;
}
...
int html_parser_cmp_tag(HTMLSTREAMPARSER *hsp, char *p, size_t l) {
    if (html_parser_tag_length(hsp) == l) if (strncmp(p, hsp->tag_name, l) == 0) return 1;
    return 0;
}

void html_parser_set_attr_buffer(HTMLSTREAMPARSER *hsp, char *buffer, size_t length) {
    hsp->attr_name = buffer;
    hsp->attr_name_max_len = length;
}
...
size_t html_parser_inner_text_length(HTMLSTREAMPARSER *hsp) {
    if (!hsp->html_part[HTML_INNER_TEXT]) return hsp->inner_text_len;
    else return 0;
}

size_t html_parser_inner_text_real_length(HTMLSTREAMPARSER *hsp) {
    return hsp->inner_text_real_len;
}

char* html_parser_inner_text(HTMLSTREAMPARSER *hsp) {
    return hsp->inner_text;
}

int html_parser_cmp_inner_text(HTMLSTREAMPARSER *hsp, char *p, size_t l) {
    if (html_parser_inner_text_length(hsp) == l) if (strncmp(p, hsp->inner_text, l) == 0) return 1;
    return 0;
}
